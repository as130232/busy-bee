...
**驗收條件**：
- 功能性：
  - [ ] 會議處理完成後自動產生行動項清單（會議未提及行動項時為空清單，禁止捏造）
  - [ ] 負責人與時限僅在會議明確提及時填入，否則留空
  - [ ] Dashboard 顯示跨會議的未完成行動項（含所屬會議標題）
  - [ ] 會議詳情頁「行動項」分頁顯示該場全部行動項
  - [ ] 可勾選完成 / 取消完成，狀態持久化
  - [ ] 僅本人可見與可操作（user_id 過濾）
- 錯誤處理：
  - [ ] 抽取失敗走既有 retry 機制，不影響 PRD / Tech Spec 產出
  - [ ] retry 不重複抽取已完成的會議（冪等）
...
**驗收條件**：
- 功能性：
  - [ ] 可一鍵複製當前分頁內容（Markdown 原文），成功有視覺回饋
  - [ ] 可下載當前分頁內容為 `.md` 檔（檔名含會議標題與文件類型）
  - [ ] 支援 Web Share API 的裝置（iOS Safari 等）顯示分享按鈕，可分享至任意 App
- 錯誤處理：
  - [ ] 內容為空時不顯示匯出工具列
  - [ ] 用戶取消系統分享不報錯
