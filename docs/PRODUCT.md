...
- F-SCENARIO：紀錄情境化（會議/閒聊模板，AI 依情境產生結構化摘要區塊）
...
**描述**：把單一「會議」擴充為更廣的「紀錄」概念，錄音/上傳前先選情境（v1：會議 / 閒聊），AI 依情境產生**結構化摘要區塊**（會議：摘要／決議事項／討論主題；閒聊：重點摘要／結論／後續待辦）。底層擷取管線（STT + 分講者）情境無關；情境只換 LLM prompt 模板與輸出區塊定義（B-lite）。日後加面試等情境 = 新增一個 prompt 模板 + 區塊定義，不動前端與管線。

**前置功能**：F-PIPELINE、F-DOCGEN

**驗收條件**：
- 功能性：
  - [x] 錄音/上傳前可選情境（會議/閒聊），預設會議；排程建立亦可選
  - [x] 詳情頁依情境顯示對應結構化區塊（通用渲染器，一套邏輯服務所有情境）
  - [x] 清單以標籤標示情境（位於標題前，與時長樣式區隔）
  - [x] 錄音預設標題依情境命名（「會議錄音 …」/「閒聊錄音 …」＋日期時間）
  - [ ] 裝置端 e2e：錄閒聊 → 看到閒聊區塊；錄會議 → 看到會議區塊（人工驗收）
- 邊界：
  - [x] 情境無效/空值回退會議（容忍舊資料）；舊紀錄預設 meeting
  - [x] PRD / Tech Spec 改為選用、不再自動產生（每筆 LLM 呼叫 3→2 次，降成本）

**邊界與限制**：
- v1 只做會議/閒聊兩情境 + 通用 fallback；面試等其他情境列後續
- 情境於建立時決定（排程編輯不改情境）；不做 LLM 自動偵測情境
- 閒聊/面試錄音的同意提示屬產品文案，列後續
