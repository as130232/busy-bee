...
**驗收條件**：
- 功能性：
  - [ ] 使用 Google 帳號可完成登入並進入 Dashboard
  - [ ] 首次登入後 users 表有對應記錄，firebase_uid 唯一
  - [ ] 重複登入不建立重複用戶（upsert）
- 錯誤處理：
  - [ ] 未帶有效 Firebase JWT 的 API 請求回 401
  - [ ] JWT 過期時回 401，前端引導重新登入
...
**驗收條件**：
- 功能性：
  - [ ] 錄音可開始 / 暫停 / 結束，結束後產生音訊檔並自動進入上傳流程
- 錯誤處理：
  - [ ] 錄音進行中離開或重新整理頁面時，顯示資料遺失警告
  - [ ] 麥克風權限被拒時顯示明確指引
- 相容性：
  - [ ] Chrome 桌面版可完成錄音（webm/opus）
...
**驗收條件**：
- 功能性：
  - [ ] 支援 .mp3 / .m4a / .webm / .wav 上傳
  - [ ] 200MB 以內檔案可完成上傳
  - [ ] 上傳完成後 meeting 進入 pending 狀態
- 錯誤處理：
  - [ ] 超過 200MB 被拒且顯示明確訊息
  - [ ] 非音訊 content-type 被上傳端點拒絕
  - [ ] 上傳中斷後可重試（重新取得上傳連結）
...
**驗收條件**：
- 功能性：
  - [ ] 上傳完成後狀態依序推進：pending → transcribing →（後續階段）
  - [ ] 90 分鐘內的中文 / 中英夾雜音訊可產出 transcript
  - [ ] 超過 STT 服務大小上限的檔案自動壓縮後轉錄
- 錯誤處理：
  - [ ] STT 失敗自動 retry，達上限後標 failed 並記錄 error_message
  - [ ] retry 不重複執行已完成的階段（冪等）
...
**驗收條件**：
- 功能性：
  - [ ] 每次成功處理產生恰好兩份 artifacts（prd、tech_spec）
  - [ ] 產出為結構化 Markdown，含固定章節骨架
  - [ ] 中文會議產出中文文件
- 錯誤處理：
  - [ ] LLM 失敗自動 retry，達上限後標 failed
  - [ ] retry 不產生重複 artifacts（冪等）
...
**驗收條件**：
- 功能性：
  - [ ] 狀態變更後前端 3 秒內收到更新（正常網路）
  - [ ] 斷線後自動重連，重連後補齊到最新狀態
  - [ ] 只收到自己 meeting 的通知
- 錯誤處理：
  - [ ] 未通過 JWT 驗證的 WebSocket 連線收不到任何資料
...
- 功能性：
  - [ ] 列表按建立時間倒序顯示本人全部會議
  - [ ] 詳情頁顯示 transcript 與兩份文件（Markdown 渲染）
  - [ ] 只顯示本人的會議
- 錯誤處理：
  - [ ] failed 的會議顯示錯誤原因與重試按鈕
...
**驗收條件**：
- 功能性：
  - [ ] 關鍵字命中 title 或 transcript 即出現在結果
  - [ ] 只搜尋本人的會議
- 錯誤處理：
  - [ ] 空關鍵字回傳一般列表，不報錯
...
**驗收條件**：
- 功能性：
  - [ ] 可建立 / 編輯含 scheduled_at 的未來會議
  - [ ] 會議前 15 分鐘（暫定，見 Q3）收到推播
  - [ ] 修改 scheduled_at 後舊提醒不觸發、新提醒生效
  - [ ] 取消推播訂閱後不再收到推播
- 相容性：
...
| 可用性 | 後端常駐一個 instance（min-instances=1）；內部工具，無正式 SLA |
...
| 編號 | 問題 | 影響範圍 | 確認對象 | 期限 |
|------|------|---------|---------|------|
| Q1 | 登入是否限制白名單（任何 Google 帳號皆可 vs 僅指定成員）？ | F-AUTH 權限設計 | Charles | M1-A 開發前 |
| Q2 | PRD / Tech Spec 的章節模板內容？ | F-DOCGEN prompt 設計 | Charles | M2-A 開發前 |
| Q3 | 提醒提前時間固定 15 分鐘，或可由用戶設定？ | F-REMIND | Charles | M2-B 開發前 |
